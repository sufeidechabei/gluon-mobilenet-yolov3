...
class YOLOV3PrefetchTargetGenerator(gluon.Block):
    ...
    def forward(self, img, xs, anchors, offsets, gt_boxes, gt_ids):
        ...
        with autograd.pause():
            ...
            class_targets = nd.one_hot(
                objectness.squeeze(
                    axis=-1), depth=self._num_class)
            ...
            # should not be a problem right now. Switch to better solution is
            # needed.
            for b in range(matches.shape[0]):
                for m in range(matches.shape[1]):
                    ...
                    center_targets[b, index, match, 0] = gtx / \
                        orig_width * width - loc_x  # tx
                    center_targets[b, index, match, 1] = gty / \
                        orig_height * height - loc_y  # ty
                    scale_targets[b, index, match, 0] = np.log(
                        gtw / np_anchors[match, 0])
                    scale_targets[b, index, match, 1] = np.log(
                        gth / np_anchors[match, 1])
                    weights[b, index, match, :] = 2.0 - \
                        gtw * gth / orig_width / orig_height
                    objectness[b, index, match, 0] = 1
                    class_targets[b, index, match, :] = 0
                    class_targets[b, index, match, int(np_gt_ids[b, m, 0])] = 1
            # since some stages won't see partial anchors, so we have to slice
            # the correct targets
            objectness = self._slice(objectness, num_anchors, num_offsets)
            center_targets = self._slice(
                center_targets, num_anchors, num_offsets)
            scale_targets = self._slice(
                scale_targets, num_anchors, num_offsets)
            weights = self._slice(weights, num_anchors, num_offsets)
            class_targets = self._slice(
                class_targets, num_anchors, num_offsets)
...
class YOLOV3DynamicTargetGeneratorSimple(gluon.HybridBlock):
    ...
    def hybrid_forward(self, F, box_preds, gt_boxes):
        ...
        with autograd.pause():
            ...
            objness_t = F.zeros_like(
                box_preds.slice_axis(
                    axis=-1, begin=0, end=1))
            center_t = F.zeros_like(
                box_preds.slice_axis(
                    axis=-1, begin=0, end=2))
            scale_t = F.zeros_like(
                box_preds.slice_axis(
                    axis=-1, begin=0, end=2))
            weight_t = F.zeros_like(
                box_preds.slice_axis(
                    axis=-1, begin=0, end=2))
            ...
            objness_t = (ious_max > self._ignore_iou_thresh) * - \
                1  # use -1 for ignored
...
class YOLOV3TargetMerger(gluon.HybridBlock):
    ...
    def __init__(self, num_class, ignore_iou_thresh, **kwargs):
        ...
        self._num_class = num_class
        self._dynamic_target = YOLOV3DynamicTargetGeneratorSimple(
            num_class, ignore_iou_thresh)
